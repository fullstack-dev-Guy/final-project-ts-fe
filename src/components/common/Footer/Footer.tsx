...
export default function Footer() {
  ...
  let guestCart: any = localStorage.getItem("cartID");

  if (guestCart === null || guestCart === " " || guestCart === undefined) {
    //console.error("cart empty");
  } else {
    setTimeout(async () => {
      const guestCart = localStorage.getItem("cartID");

      async function deleteCart(guestCart: string) {
      ...
      const resultDeleteCart = deleteCart(guestCart!);
      if (!resultDeleteCart) {
        //console.error("cart not deleted");
      } else {
        localStorage.removeItem("cartID");
        window.location.reload();
        navigate("/");
      ...
      <footer className="bg-white p-4 dark:bg-gray-900 sm:p-6">
        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          <span className="text-sm text-gray-500 dark:text-gray-400 sm:text-center">
            © 2023{" "}
            <a href="/" className="hover:underline">
              Guy Iserlis - Full Stack Final Project - SarCafe-Since-2022 ™
            </a>
            . All Rights Reserved.
          </span>
